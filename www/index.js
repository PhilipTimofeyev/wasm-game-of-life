import { Universe, Cell } from "wasm-game-of-life";
import { memory } from "wasm-game-of-life/wasm_game_of_life_bg.wasm";

let universe = Universe.new();
const width = universe.width();
const height = universe.height();

const CELL_SIZE = 10;
const GRID_COLOR = "#CCCCCC";
const DEAD_COLOR = "#FFFFFF";
const ALIVE_COLOR = "#000000";

const canvas = document.getElementById("game-of-life-canvas")
canvas.height = (CELL_SIZE + 1) * height + 1;
canvas.width = (CELL_SIZE + 1) * width + 1;

const ctx = canvas.getContext('2d')

let animationId = null;

const numOfTicks = document.getElementById("num-ticks");
const outputTicks = document.getElementById("output-ticks");
const resetRandomBtn = document.getElementById("reset-random");
const resetEmptyBtn = document.getElementById("reset-empty");

resetRandomBtn.addEventListener('click', event => {
    universe = Universe.new();
    pause()
    drawGrid();
    drawCells();
})

resetEmptyBtn.addEventListener('click', event => {
    universe = Universe.new_empty();
    pause()
    drawGrid();
    drawCells();
})

numOfTicks.addEventListener('change', event => {
    outputTicks.value = event.target.value
})

const renderLoop = () => {  
    drawGrid();
    drawCells();

    let ticks = 0
    while (ticks < numOfTicks.value) {
        universe.tick();
        ticks++
    }

    animationId = requestAnimationFrame(renderLoop);
}

const isPaused = () => {
    return animationId === null;
};

const playPauseButton = document.getElementById("play-pause");

const play = () => {
    playPauseButton.textContent = "⏸";
    renderLoop();
}

const pause = () => {
    playPauseButton.textContent = "▶";
    cancelAnimationFrame(animationId);
    animationId = null;
}

playPauseButton.addEventListener("click", event => {
    if (isPaused()) {
        play();
    } else {
        pause();
    }
})

canvas.addEventListener("click", event => {
    const boundingRect = canvas.getBoundingClientRect();

    const scaleX = canvas.width / boundingRect.width;
    const scaleY = canvas.height / boundingRect.height;

    const canvasLeft = (event.clientX - boundingRect.left) * scaleX;
    const canvasTop = (event.clientY - boundingRect.top) * scaleY;

    const row = Math.min(Math.floor(canvasTop / (CELL_SIZE + 1)), height - 1);
    const col = Math.min(Math.floor(canvasLeft / (CELL_SIZE + 1)), width - 1);

    if (event.shiftKey) {
        buildGlider(row, col)
    } else if (event.altKey) {
        buildPulsar(row, col)
    } else {
        universe.toggle_cell(row, col);
    }

    drawGrid();
    drawCells();

})

const buildGlider = (row, col) => {
    universe.toggle_cell(row - 1, col - 1);
    universe.toggle_cell(row, col);
    universe.toggle_cell(row, col + 1);
    universe.toggle_cell(row + 1, col);
    universe.toggle_cell(row + 1, col - 1);
}

const buildPulsar = (row, col) => {
    // top left
    universe.toggle_cell(row - 1, col - 2);
    universe.toggle_cell(row - 1, col - 3);
    universe.toggle_cell(row - 1, col - 4);
    
    universe.toggle_cell(row - 6, col - 2);
    universe.toggle_cell(row - 6, col - 3);
    universe.toggle_cell(row - 6, col - 4);

    universe.toggle_cell(row - 2, col - 6);
    universe.toggle_cell(row - 3, col - 6);
    universe.toggle_cell(row - 4, col - 6);
    
    universe.toggle_cell(row - 2, col - 1);
    universe.toggle_cell(row - 3, col - 1);
    universe.toggle_cell(row - 4, col - 1);

    // bottom right
    universe.toggle_cell(row + 1, col + 2);
    universe.toggle_cell(row + 1, col + 3);
    universe.toggle_cell(row + 1, col + 4);
    
    universe.toggle_cell(row + 6, col + 2);
    universe.toggle_cell(row + 6, col + 3);
    universe.toggle_cell(row + 6, col + 4);

    universe.toggle_cell(row + 2, col + 6);
    universe.toggle_cell(row + 3, col + 6);
    universe.toggle_cell(row + 4, col + 6);
    
    universe.toggle_cell(row + 2, col + 1);
    universe.toggle_cell(row + 3, col + 1);
    universe.toggle_cell(row + 4, col + 1);

    // top right
    universe.toggle_cell(row - 1, col + 2);
    universe.toggle_cell(row - 1, col + 3);
    universe.toggle_cell(row - 1, col + 4);
    
    universe.toggle_cell(row - 6, col + 2);
    universe.toggle_cell(row - 6, col + 3);
    universe.toggle_cell(row - 6, col + 4);

    universe.toggle_cell(row - 2, col + 6);
    universe.toggle_cell(row - 3, col + 6);
    universe.toggle_cell(row - 4, col + 6);
    
    universe.toggle_cell(row - 2, col + 1);
    universe.toggle_cell(row - 3, col + 1);
    universe.toggle_cell(row - 4, col + 1);

    // bottom left
    universe.toggle_cell(row + 1, col - 2);
    universe.toggle_cell(row + 1, col - 3);
    universe.toggle_cell(row + 1, col - 4);
    
    universe.toggle_cell(row + 6, col - 2);
    universe.toggle_cell(row + 6, col - 3);
    universe.toggle_cell(row + 6, col - 4);

    universe.toggle_cell(row + 2, col - 6);
    universe.toggle_cell(row + 3, col - 6);
    universe.toggle_cell(row + 4, col - 6);
    
    universe.toggle_cell(row + 2, col - 1);
    universe.toggle_cell(row + 3, col - 1);
    universe.toggle_cell(row + 4, col - 1);
}

const drawGrid = () => {
    ctx.beginPath();
    ctx.strokeStyle = GRID_COLOR;

    for (let i = 0; i <= width; i++) {
        ctx.moveTo(i * (CELL_SIZE + 1) + 1, 0);
        ctx.lineTo(i * (CELL_SIZE + 1) + 1, (CELL_SIZE + 1) * height + 1);
    }

    for (let j = 0; j <= height; j++) {
        ctx.moveTo(0, j * (CELL_SIZE + 1) + 1);
        ctx.lineTo((CELL_SIZE + 1) * width + 1, j * (CELL_SIZE + 1) + 1)
    }

    ctx.stroke();
};

const getIndex = (row, column) => {
    return row * width + column
};

const bitIsSet = (n, arr) => {
    const byte = Math.floor(n / 8);
    const mask = 1 << (n % 8);
    return (arr[byte] & mask) === mask;
  };

const drawCells = () => {
    const cellsPtr = universe.cells();
    const cells = new Uint8Array(memory.buffer, cellsPtr, width * height / 8);

    ctx.beginPath()

    for (let row = 0; row < height; row++) {
        for (let col = 0; col < width; col++) {
            const idx = getIndex(row, col);

            ctx.fillStyle = bitIsSet(idx, cells)
                ? ALIVE_COLOR
                : DEAD_COLOR;

            ctx.fillRect(
                col * (CELL_SIZE + 1) + 1,
                row * (CELL_SIZE + 1) + 1,
                CELL_SIZE,
                CELL_SIZE
            );
        }
    }
    ctx.stroke();
};

drawGrid();
drawCells();
play();